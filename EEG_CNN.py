...
def CNN(reg=0, pool_size=3, use_max_pool=True, dropout=0.5, dropout_on_conv=False, filters=[10 for _ in range(4)], affine_layer_sizes=[], num_filters=[25, 50, 100, 200], input_shape=(500, 1, 22), output_classes=4):
    ...
    for filter, num_filter in zip(filters, num_filters):
        model.add(Conv2D(filters=num_filter, kernel_size=(filter,1), padding='same', activation='relu', input_shape=input_shape, kernel_regularizer=regularizers.L2(reg)))
        if use_max_pool:
